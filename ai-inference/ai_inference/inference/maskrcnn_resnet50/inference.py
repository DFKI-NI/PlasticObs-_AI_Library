...
class MaskRcnnInference(ModelInference):
    model = None
    ...
    def warm_up(
        ...
        with open("ai_inference/inference/maskrcnn_resnet50/categories.json") as f:
            data = json.load(f)
    ...
    def _do_predict(self, input_file: str, input_parameters: PredictInput) -> gpd.GeoDataFrame:
        ...
        with Image.open(input_file).convert("RGB") as image:
            ...
            output = MaskRcnnInference.model(trans_image)  # type: ignore
            ...
            cats = [MaskRcnnInference.categories[int(l)-1] for l in labels]
...
if __name__ == '__main__':
    # python -m ai_inference.inference.maskrcnn_resnet50.inference
    ...
    result_gdf.to_file("Shape_files/maskrcnn_bboxes_with_metadata.shp")
