...
class Decoder(nn.Module):
    ...
    def reshape_function(self, x, shape_before_flattening):
        return x.view(-1, *shape_before_flattening)
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        # reshape the tensor
        x = self.reshape_function(x, self.shape_before_flattening)
...
def predict_single(model, test_image, DEVICE):
    ...
    img = torch.Tensor(img.reshape(-1, test_image.shape[0], test_image.shape[1], test_image.shape[2])).permute(0, 3, 1, 2).to(DEVICE)
...
if __name__=='__main__':
    image_size = (3200, 480)  # Example image size
    embedding_dim = 35  # Example embedding dimension
    encoder = Encoder(image_size, embedding_dim)
    input_tensor = torch.randn(32, 3, image_size[0], image_size[1])  # Example input
